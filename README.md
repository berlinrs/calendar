...
This repository defines the calendars for the Rust Berlin group.
Calendars per event are defined in their respective TOML files and a iCalendar file will be generated
and hosted on GitHub Pages for contributors and interested parties to add to their calendar application of choice.
...
`https://berlinrs.github.io/calendar/$name.ics`.
Below are links to the `ics` files for all the current calendars in this repository:
...
* [All](https://berlinrs.github.io/calendar/all.ics)
  ...
  * [Rust Berlin on Location](https://berlinrs.github.io/calendar/on-location.ics)

You can copy these links and import them into your calendar application of choice.

## How do I add a calendar?

Add a new file in the repository with an appropriate name.
Add the path to the `meta.includes` list in `all.toml`.
...
Each event have a globally UID (because calendars can be included in other calendars, they must be globally unique for this whole repository).
It should never be changed after the calendar has been published,
see [How to generate a UID for an event?](#how-do-i-generate-a-uid-for-a-new-event).
...
- and make sure to update `last_modified_on`.
Some clients use `last_modified_on` to decide whether to publish an update to an event or recurrence or not.
To ensure the client will publish the event, set `last_modified_on` to a value in the future (a few days past the current date should be enough).
...
If the event isn't recurring, then you don't need to remove it, it'll just stay in the calendar in the past.
If the event is incorrect, you can always update it, see [*How do I update an event?*][update].
...
You can see the [`example.toml`][example] from the [`calendar-generation`][calendar_generation] repository for a complete list of all supported configuration options.
...
[schema]: https://github.com/berlinrs/calendar/tree/main#what-is-the-schema-for-the-calendars
[subscribe]: https://github.com/berlinrs/calendar/tree/main#how-do-i-subscribe-to-these-calendars
[update]: https://github.com/berlinrs/calendar/tree/main#how-do-i-update-an-event

## Testing and validating calendar files

To test that your calendar changes work, you can install the calendar generation tool and generate the `all.ics` calendar
(which should include every other calendar, but you can run it on a specific file too).
...
Based on [rust-lang/calendar](https://github.com/rust-lang/calendar).
